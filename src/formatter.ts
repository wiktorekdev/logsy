import { ansi, colorize } from "./ansi.js";
...
export function format(entry: LogEntry, noColor = false): string {
  const cfg = LEVEL_CONFIG[entry.level];

  const ts = colorize(formatTimestamp(entry.timestamp), ansi.dim, ansi.gray);
  ...
  // Strip colors when the caller disabled them or when stdout isn't a TTY
  return noColor ? line.replace(/\x1b\[[0-9;]*m/g, "") : line;
}
